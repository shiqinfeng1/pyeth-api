...
contract TwitterAccount is  Owned {
    ...
    event Log_lotus_result(bytes32 _id, bytes32[] users,address[] users_addr);
    ...
    */
    function lotus(uint luckyboys_num, bytes32 retweet_id, bytes32[] users_list) public onlyOwner {
        ...
        bytes32[] memory valid_users_list; //转发的用户id中,已绑定地址的用户
        address[] memory users_addr; //转发的用户id中,已绑定地址的用户的以太坊地址
        bytes32[] memory luckyboys;  //中奖用户id
        address[] memory luckyboys_addr; //中奖用户地址
        ...
        for(uint i=0;i<users_list.length;i++){
            if(accounts[users_list[i]]!=0){
                users_addr[i] = accounts[users_list[i]];
                valid_users_list[i] = users_list[i];
            }
        }
        //随机抽取中奖用户
        uint total_users = users_addr.length;
